...
const addStockLocationButton = document.getElementById("addStockLocation");
...
function fillStockLocationRows(locations = []) {
    const wrapper = document.getElementById("stockLocationsWrapper");
    wrapper.innerHTML = "";

    locations.forEach(loc => {
        const row = createStockLocationRow(loc.shelfCode, loc.quantity);
        wrapper.appendChild(row);
    });

    if (locations.length === 0) {
        wrapper.appendChild(createStockLocationRow());
    }
}
...
async function openEditModal(row) {
    ...
    fillStockLocationRows(stock.stockLocations);
    ...
    filteredStocks = allStocks.filter(stock => {
        ...
        filters.forEach((filter, colIdx) => {
            ...
            switch (colIdx) {
                ...
                case 7: fieldValue = stock.driveType; break;
                case 8: fieldValue = stock.transmissionStatusName; break;
                case 9: fieldValue = stock.shelfSummary; break;
...
recordModal.addEventListener('shown.bs.modal', function () {
    $('#recordModal select.select2').each(function () {
        if (!$(this).hasClass('select2-hidden-accessible')) {
            $(this).select2({ dropdownParent: $('#recordModal'), width: '100%', theme: 'bootstrap-5' });
        }
    });
});
...
addNewBtn.addEventListener("click", () => {
    ...
    if (typeof fillStockLocationRows === 'function') {
        ...
        if (wrapper) {
            wrapper.innerHTML = "";
            if (typeof createStockLocationRow === 'function') {
                wrapper.appendChild(createStockLocationRow());
            }
        }
    }
...
async function populateSelectFields() {
    try {
        ...
        globalShelfCodes = data.shelfCodes || [];
        sessionStorage.setItem("allVehicleModels", JSON.stringify(data.vehicleModels));
    } catch (err) {
        Swal.fire({
            icon: 'error',
            title: 'Hata!',
...
addStockLocationButton.addEventListener("click", () => {
    const wrapper = document.getElementById("stockLocationsWrapper");
    wrapper.appendChild(createStockLocationRow());
});
...
saveButton.addEventListener("click", async function (event) {
    ...
    // 1) Lokasyonları oku
    const stockLocationRows = document.querySelectorAll(".stock-location-row");
    const stockLocations = Array.from(stockLocationRows).map(row => ({
        shelfCode: row.querySelector(".shelf-select").value,
        quantity: parseInt(row.querySelector(".quantity-input").value || "0", 10)
    }));

    // 2) Toplam miktar
    const totalQuantity = stockLocations.reduce((sum, loc) => sum + (loc.quantity || 0), 0);

    // 3) Zorunlu alan kontrolü (formu hedefle)
    ...
    const data = {
        ...
        stockLocations: totalQuantity === 0 ? [] : stockLocations,
