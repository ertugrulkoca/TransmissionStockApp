...
async function deleteModel() {
    const id = vehicleModelId.value;
    if (!id) {
        await Swal.fire({
            icon: 'warning',
            title: 'Uyarı',
            text: "Önce bir model seçin."
        });
        return;
    }
    ...
    const confirmResult = await Swal.fire({
        icon: 'question',
        title: 'Onay',
        text: 'Bu modeli silmek istediğinize emin misiniz?',
        showCancelButton: true,
        confirmButtonText: 'Evet',
        cancelButtonText: 'Hayır'
    });
    ...
    try {
        ...
        if (res.ok) {
            ...
            clearModelForm();
            updateVehicleModelSelect(vehicleBrandId.value);
        } else {
            await Swal.fire({
                icon: 'error',
                title: 'Hata',
                text: "Model silinemedi."
            });
        }
    } catch (error) {
        await Swal.fire({
...
async function saveModel(e) {
    ...
    const brandId = vehicleBrandId.value;

    if (!brandId) {
        await Swal.fire({
            icon: 'warning',
            title: 'Uyarı',
            text: "Önce bir araç markası seçin."
        });
        return;
    }
    if (!name) {
        await Swal.fire({
            icon: 'warning',
            title: 'Uyarı',
            text: "Model adı boş olamaz."
        });
        return;
    }

    const method = id ? 'PUT' : 'POST';
    const url = id ? `/api/VehicleModel/${id}` : `/api/VehicleModel`;
    ...
    try {
        ...
        if (res.ok) {
            ...
            await Swal.fire({
                icon: 'error',
                title: 'Hata',
                text: "Model kaydetme işlemi başarısız."
            });
        }
    } catch (error) {
        await Swal.fire({
