...
namespace TransmissionStockApp.Controllers
{
    ...
    public class TransmissionStockLocationController : ControllerBase
    {
        ...
        {
            var result = await _service.UpdateQuantityAsync(dto.TransmissionStockId, dto.StockLocationId, dto.Quantity);
            if (!result.Success)
                return NotFound(result.ErrorMessage);
        ...
        [HttpDelete]
        public async Task<IActionResult> Delete(int transmissionStockId, int stockLocationId)
        {
            var result = await _service.DeleteAsync(transmissionStockId, stockLocationId);
            if (!result.Success)
                return NotFound(result.ErrorMessage);
